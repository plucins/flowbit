...
Custom scope values are matched against existing `.flowbit/docs/standards/*/` directories and filter analysis to relevant files.
...
2. **Check prerequisites**: Verify `.flowbit/docs/` exists. If not, offer to run `/flowbit-spec-init` first
...
| Situation | Strategy |
|-----------|----------|
| `.flowbit/docs/` missing | Offer `/flowbit-spec-init`, abort if declined |
